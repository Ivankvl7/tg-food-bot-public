...
class B2BInstance:
    def __init__(self, bucket_name: str = CONFIG.b2b.bucket_name) -> None:
        ...
        self.bucket: Bucket = self.get_bucket_api()
    ...
    @staticmethod
    def get_destination_path(product_id: int,
    ...
    def upload_file(self,
        ...
        data_bytes: bytes = self.get_media_content(url)
        file_name: str = self.generate_media_name(product_id=product_id,
                                                  media_type=media_type,
                                                  file_id=file_id)
        uploaded_file: FileVersion = self.bucket.upload_bytes(data_bytes=data_bytes,
                                                              file_name=file_name)

    def get_static_data(self,
                        product_id: int,
                        media_type: StaticContentType = StaticContentType.IMAGE) -> Any:
